...
})({"hTCtI":[function(require,module,exports,__globalThis) {
...
startButton.addEventListener('click', ()=>{
    ...
    hideMessages();
    if (!isStarting) startMessage.classList.remove('hidden');
    updateField();
    updateScore();
    checkGameStatus();
});
...
function updateField() {
    ...
    game.getState().forEach((row)=>{
        ...
        row.forEach((value)=>{
            ...
            if (value !== 0) td.textContent = value;
            else td.textContent = '';
...
class Game {
    ...
    moveUp() {
        ...
        this.move((_, columnIndex)=>this.getColumn(columnIndex), (column, columnIndex)=>this.setColumn(column, columnIndex));
    }
    moveDown() {
        if (this.status !== Game.STATUS.playing) return;
        this.move((_, columnIndex)=>this.getColumn(columnIndex).reverse(), (column, columnIndex)=>this.setColumn(column.reverse(), columnIndex));
    }
    move(getRow, setRow) {
        if (this.status !== Game.STATUS.playing) return;
        let moved = false;
    ...
    canMove() {
        for(let row = 0; row < 4; row++)for(let column = 0; column < 4; column++){
            ...
            if (column < 3 && this.state[row][column] === this.state[row][column + 1]) return true;
            if (row < 3 && this.state[row][column] === this.state[row + 1][column]) return true;
        }
        return false;
    }
    checkLoseOrWin() {
...
},{}]},["hTCtI","3cYfC"], "3cYfC", "parcelRequire94c2")
